...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        model_channels = denoised_fn.get('model_channels', Rr) if denoised_fn is not None else Rr
        if noise is not None:
            img = noise
        else:
            img = th.randn((Bb, model_channels, Hh, Ww), device=device)
        ...
        for iteration, (i, j) in pbar:
            ...
            with th.enable_grad():
                ...
                for _ in range(inner_steps):
                    ...
                    latent_u = adapter_model(pred_xstart_u) if adapter_model is not None else pred_xstart_u
            ...
            # update
            E_tune = factor_model()
            latent = adapter_model(pred_xstart) if adapter_model is not None else pred_xstart
            xhat = (latent + 1) / 2
