...
best_cfg=""
run_id=0
log_file="grid_inpainting_vanilla.log"
result_file="grid_inpainting_vanilla_results.tsv"
: > "${log_file}"
: > "${result_file}"
...
run_config() {
  ...
  echo "[VANILLA][inpainting][run ${run_id}] eta1=${eta1} eta2=${eta2} k=${k} step=${step} rank=${rank}"
...
PY
  ...
  echo "[VANILLA][inpainting][run ${run_id}] psnr=${run_psnr} | best_psnr=${best_psnr}"
  rm -f "${run_log}"
}
...
echo "[VANILLA][inpainting] mode=coarse2fine, top_k=${TOP_K}, seed=${RANDOM_SEED}"
run_coarse_to_fine

echo "[VANILLA][inpainting] done"
echo "[VANILLA][inpainting] best_psnr=${best_psnr}"
echo "[VANILLA][inpainting] best_cfg=${best_cfg}"
