...
best_cfg=""
run_id=0
log_file="grid_denoise_vanilla.log"
result_file="grid_denoise_vanilla_results.tsv"
: > "${log_file}"
: > "${result_file}"
...
run_config() {
  ...
  echo "[VANILLA][denoise][run ${run_id}] eta1=${eta1} eta2=${eta2} k=${k} step=${step} rank=${rank}"
  ...
  if [[ "${run_status}" == "failed" ]]; then
    if grep -qiE "outofmemoryerror|cuda out of memory" "${run_log}"; then
      echo "[VANILLA][denoise][run ${run_id}] OOM detected, skip."
      rm -f "${run_log}"
      sleep 2
      return 0
    fi
    echo "[VANILLA][denoise][run ${run_id}] failed (non-OOM), stop."
    rm -f "${run_log}"
    exit 1
  fi
...
PY
  ...
  echo "[VANILLA][denoise][run ${run_id}] psnr=${run_psnr} | best_psnr=${best_psnr}"
  rm -f "${run_log}"
}
...
echo "[VANILLA][denoise] mode=coarse2fine, top_k=${TOP_K}, seed=${RANDOM_SEED}"
run_coarse_to_fine

echo "[VANILLA][denoise] done"
echo "[VANILLA][denoise] best_psnr=${best_psnr}"
echo "[VANILLA][denoise] best_cfg=${best_cfg}"
